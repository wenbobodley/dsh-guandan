...
/** 快照里的索引串（"4,5,8,…"）→ 可读牌名（按点数降序），如 "♥5 ♥6 ♥6 ♠A" */
function idxStrToNames(s) {
  if (!s) return '';
  const list = s.split(',').filter(Boolean).map((x) => Number(x));
  list.sort((a, b) => cardOrder(b) - cardOrder(a));
  return list.map((x) => numToCard(x)).join(' ');
}
function cardOrder(i) {
  if (i >= 52) return i; // 52 小王、53 大王
  const r = i % 13;
  return r === 0 ? 1 : r + 1; // 2 最小(1) … A 最大(13)
}
...
function renderGame(view, extra = []) {
  ...
  lines.push(`你的手牌（${view.me.count} 张）：${idxStrToNames(view.me.cards)}`);
  for (const t of s.tributeLog) lines.push(`📤 ${t}`);
  if (s.lastMax) {
    lines.push(`当前最大：${s.hands[s.lastMax.player]?.name ?? '?'} 出 ${idxStrToNames(s.lastMax.cards)}（${typeName(s.lastMax.type)}）`);
  }
  for (const h of s.history) {
    lines.push(h.pass ? `· ${s.hands[h.player]?.name} 过` : `· ${s.hands[h.player]?.name} 出 ${idxStrToNames(h.cards)}`);
  }
  lines.push(...extra);
...
export function apply(ctx) {
  ...
  ctx.tools.register({
    name: 'guandan_play',
    description: `在${BRAND}牌局中出牌或过牌。cards 传入要出的牌（如 "H2 H2 S2" 或 "33 44 55"），pass=true 表示过牌。非法出牌会返回错误并给出提示。牌友会自动接续行牌直到轮到你。`,
    parameters: {
      type: "object",
      properties: {
