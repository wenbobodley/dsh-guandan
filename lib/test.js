...
import { strToCards, cardsToVector, fullDeck } from './cards.js';
...
console.log('== 牌组组成（两副牌 108 张）==');
{
  const deck = fullDeck();
  ok('牌组共 108 张', deck.length === 108, `实际 ${deck.length}`);
  const counts = {};
  deck.forEach((c) => { counts[c] = (counts[c] || 0) + 1; });
  const max = Math.max(...Object.values(counts));
  ok('每种牌最多 2 张', max <= 2, `实际最多 ${max}`);
  // 4 张 ♥J 不可能出现在正常牌局 → 需能组 4 炸（用 2 张♥J + 其他花色的 J）
  const acts = legalActions(hand('HJ', 'HJ', 'SJ', 'CJ'), '2');
  ok('2×♥J+SJ+CJ 可组 J 4炸', acts.some((a) => a.type === 4 && a.value === 9));
}
...
console.log('== 顺子/连对/钢板/同花顺 ==');
{
  ...
  // 回归：打 J 时手里只有一张 J♣（无逢人配），顺子 8-9-10-J-Q 必须可出（级牌自然参与）
  const actsLv = legalActions(hand('D8', 'H9', 'ST', 'CJ', 'DQ', 'D5', 'C7', 'SA'), 'J');
  ok('级牌J仅一张实体牌时顺子 8910JQ 可出', actsLv.some((a) => a.type === 10 && eqVec(a.cards, hand('D8', 'H9', 'ST', 'CJ', 'DQ'))));
  // 打 2 时 A2345（含级牌2自然参与）可出
  const actsA = legalActions(hand('SA', 'C2', 'D3', 'C4', 'S5'), '2');
  ok('A2345(含级牌2自然参与) 可出', actsA.some((a) => a.type === 10 && eqVec(a.cards, hand('SA', 'C2', 'D3', 'C4', 'S5'))));
  // 连对：334455
