...
/**
 * 单步推进（UI 节奏用）：只让当前机器人走一手，或处理局间自动续局。
 * 客户端每 5 秒调用一次，形成"每个机器人至少隔 5 秒出手"的行牌节奏。
 * @returns {{done: boolean, notes: string[]}} done=true 表示已轮到人类或整局通关
 */
export function advanceOne(game) {
  if (game.roundEnd) {
    if (game.episodeEnd) return { done: true, notes: [] };
    const winners = game.winOrder.map((i) => game.players[i].name).join('、');
    const prev = game.roundNo;
    game.startRound();
    return { done: false, notes: [`🔚 第${prev}局结束（头游：${winners}）→ 自动进入第${game.roundNo}局（级牌 ${game.curRank}）`] };
  }
