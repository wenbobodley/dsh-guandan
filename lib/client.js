	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector('style[data-plugin-css="' + STYLE_ID + '"]') === null) {
			...
			tag.textContent = [
				...
				".gd-seat-info{font-weight:700;font-size:14px;color:#e2f0e8}",
		...
		function GuandanBoard(props) {
			...
			var center = h("div", { className: "gd-center" },
				h("div", { className: "gd-center-round" }, "第" + game.round + "局 · 级牌 " + game.level + "（逢人配=红桃" + game.level + "）"),
			...
			var table = h("div", { className: "gd-table" },
				h("div", { className: "gd-seat gd-seat-north" },
					h("div", { className: "gd-seat-info" + (2 === partner ? " gd-partner" : "") }, "⬆ 北 · " + (north ? north.name : "") + (2 === partner ? "（对家）" : "") + " · " + (north ? north.count : 0) + " 张"),
					nT ? (nT.pass ? h("div", { className: "gd-pass-mark" }, "过") : h("div", { className: "gd-plays" }, commaList(nT.cards).slice(0, 6).map(function (c) { var p = cardParts(c); return h("span", { className: "gd-mini " + colorClass(c), key: c }, h("span", { className: "gd-mini-rank" }, p.rank), h("span", { className: "gd-mini-suit" }, p.suit)); }))) : null),
				h("div", { className: "gd-mid" },
					h("div", { className: "gd-seat gd-seat-west" },
						h("div", { className: "gd-seat-info" }, "⬅ 西 · " + (west ? west.name : "") + " · " + (west ? west.count : 0) + " 张"),
						wT ? (wT.pass ? h("div", { className: "gd-pass-mark" }, "过") : h("div", { className: "gd-plays" }, commaList(wT.cards).slice(0, 6).map(function (c) { var p = cardParts(c); return h("span", { className: "gd-mini " + colorClass(c), key: c }, h("span", { className: "gd-mini-rank" }, p.rank), h("span", { className: "gd-mini-suit" }, p.suit)); }))) : null),
					center,
					h("div", { className: "gd-seat gd-seat-east" },
						h("div", { className: "gd-seat-info" }, "➡ 东 · " + (east ? east.name : "") + " · " + (east ? east.count : 0) + " 张"),
						eT ? (eT.pass ? h("div", { className: "gd-pass-mark" }, "过") : h("div", { className: "gd-plays" }, commaList(eT.cards).slice(0, 6).map(function (c) { var p = cardParts(c); return h("span", { className: "gd-mini " + colorClass(c), key: c }, h("span", { className: "gd-mini-rank" }, p.rank), h("span", { className: "gd-mini-suit" }, p.suit)); }))) : null)),
				h("div", { className: "gd-seat gd-seat-south" },
					h("div", { className: "gd-seat-info gd-me" }, "⬇ 南 · 你 · " + (south ? south.count : 0) + " 张"),
					sT ? (sT.pass ? h("div", { className: "gd-pass-mark" }, "过") : h("div", { className: "gd-plays" }, commaList(sT.cards).slice(0, 6).map(function (c) { var p = cardParts(c); return h("span", { className: "gd-mini " + colorClass(c), key: c }, h("span", { className: "gd-mini-rank" }, p.rank), h("span", { className: "gd-mini-suit" }, p.suit)); }))) : null));
				...
				groups.map(function (grp) {
					return h("span", { className: "gd-group", key: grp.rank },
						...
						grp.cards.map(function (c) {
							var p = cardParts(c);
							return h("span", {
								className: "gd-card " + colorClass(c) + (selected.indexOf(c) >= 0 ? " sel" : ""),
			...
			var actions = h("div", { className: "gd-actions" },
				h("button", { className: "gd-big gd-act-play", onClick: onPlay, disabled: busy || !humanTurn || selected.length === 0 }, "出牌 (" + selected.length + ")"),
				h("button", { className: "gd-big gd-act-pass", onClick: onPass, disabled: busy || !humanTurn }, "过牌"),
				h("button", { className: "gd-big gd-act-clear", onClick: function () { setSelected([]); }, disabled: selected.length === 0 }, "清空"),
			...
			return h("div", { className: "gd-board", style: { "--gd-card-h": cardH + "px" } },
				...
				handEl,
				actions,
				h("div", { className: "gd-hint" }, humanTurn ? "点牌选中 → 出牌；超时 30 秒自动过牌" : "牌友行牌中…"),
