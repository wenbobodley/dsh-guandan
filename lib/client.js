...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---- 牌桌（浮层内全量渲染）----
		// 牌面尺寸档位：高度 px（持久化到 localStorage）
		var CARD_SIZES = [44, 54, 72, 92];
		var CARD_SIZE_LABELS = ["小", "中", "大", "特大"];
		function loadCardH() {
			try { var n = Number(localStorage.getItem("gd-card-h")); return CARD_SIZES.indexOf(n) >= 0 ? n : 54; } catch (e) { return 54; }
		}
