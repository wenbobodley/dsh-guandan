...
export class GuandanGame {
  ...
  play(playerId, action) {
    ...
    if (isPass) {
      this.trickPass += 1;
      this.history.push({ player: playerId, pass: true });
    } else {
      this.trickPass = 0;
      ...
      this.history.push({ player: playerId, cards: action.cards, type: action.type, value: action.value });
    }
    if (this.wind && !isPass) {
      this.wind = false;
      this.windNum = 0;
  ...
  snapshot(humanId) {
    ...
    return {
      ...
      history: this.history.slice(-8).map((h) =>
        h.pass ? { player: h.player, pass: true } : { player: h.player, cards: cardVecStr(h.cards) }
      ),
    };
  }
}
