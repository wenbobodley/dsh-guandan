...
export class GuandanGame {
  ...
  play(playerId, action) {
    ...
    if (isPass) {
      this.trickPass += 1;
      this.history.push({ player: playerId, pass: true, auto: !!action?.auto, trick: this.trickNo });
    } else {
      this.trickPass = 0;
  ...
  snapshot(humanId) {
    ...
    // 局结束：所有未出完的玩家（仍在桌上的，含末游）底牌全部公开翻出给大家看
    if (this.roundEnd) {
      for (const id of this.onTable) {
        if (id === undefined || id === null || id < 0 || id > 3) continue;
        if (!hands[id].cards) {
          hands[id].cards = cardVecStr(this.players[id].cards);
          hands[id].revealed = true; // 标记：剩余底牌已公开
        }
      }
    }
    return {
      ...
      history: this.history.slice(-8).map((h) =>
        h.pass ? { player: h.player, pass: true, auto: !!h.auto, trick: h.trick } : { player: h.player, cards: cardVecStr(h.cards), type: h.type, value: h.value, trick: h.trick }
      ),
      trickNo: this.trickNo,
      trick: this.history.filter((h) => h.trick === this.trickNo).map((h) =>
        h.pass ? { player: h.player, pass: true, auto: !!h.auto } : { player: h.player, cards: cardVecStr(h.cards), type: h.type, value: h.value }
      ),
    };
  }
}
