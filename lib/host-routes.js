...
export function uiSnapshot(game) {
  ...
  return {
    ...
    hands: s.hands.map((h) => ({
      ...
      count: h.count,
      isHuman: h.isHuman,
      ...(h.id === human && h.cards ? { cards: h.cards.split(',') } : {}),
    })),
    lastMax: s.lastMax ? { player: s.lastMax.player, name: s.hands[s.lastMax.player]?.name ?? '', cards: s.lastMax.cards, type: typeName(s.lastMax.type) } : null,
    history: s.history.map((h) =>
      h.pass ? { player: h.player, name: s.hands[h.player]?.name ?? '', pass: true } : { player: h.player, name: s.hands[h.player]?.name ?? '', cards: h.cards, type: typeName(h.type) }
    ),
