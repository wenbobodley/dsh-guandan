...
function playTypeSound(typeNameStr) {
  ...
  if (t.includes('炸弹') || t.includes('天王炸')) {
    // 炸弹：大声轰炸 + 真人发音"炸弹"
    bombSoundAt(0, 1.0);
    speakChinese('炸弹');
    return;
  ...
  if (t === '三带二') { [130.81, 130.81, 130.81, 392, 392].forEach((f, i) => tone(f, 0.15, 'triangle', 0.15, i * 0.09)); return; } // 三声低锤 + 两声短音（3+2 结构）
  if (t === '同花顺') {
    // 同花顺：连续 5 声连贯轰炸（1.2 音量，间隔 0.12s 紧凑连发）+ 0.8s 后真人发音"同花顺"
    [0, 0.12, 0.24, 0.36, 0.48].forEach((d) => bombSoundAt(d, 1.2));
    speakChinese('同花顺', 0.8);
    return;
  }
}
// —— 胜利号角：欢快上行琶音庆祝（头游赢时播放） ——
function winFanfare() {
  // C5 E5 G5 C6 G6 C6（上行庆祝旋律，间隔 0.1s 紧凑）
  const notes = [523.25, 659.25, 783.99, 1046.5, 1318.51, 1046.5];
  notes.forEach((f, i) => tone(f, 0.2, 'triangle', 0.2, i * 0.1));
  tone(1567.98, 0.55, 'triangle', 0.16, notes.length * 0.1); // 高音收尾
}
...
/** 点组牌区任意一张 = 整个组被选中（高亮），再点"出"按钮即出整组；出完 handView 自动整组清空；不允许组内单出 */
function selectGroupPlay(grp) {
  const g = state.game;
  if (!g || !grp || !grp.cards || !grp.cards.length) return;
  const inRoom = state.screen === 'room';
  // 整组选中：组内每张牌按实例选中（同名散牌不在组内的部分不选）
  const sel = {};
  for (const c of grp.cards) sel[c] = (sel[c] || 0) + 1;
  const me = g.hands ? g.hands.find((h) => inRoom ? h.id === state.viewerSeat : h.isHuman) : null;
  if (me && me.cards) {
    const tot = {};
    for (const c of me.cards) tot[c] = (tot[c] || 0) + 1;
    for (const k in sel) if (sel[k] > (tot[k] || 0)) sel[k] = tot[k] || 0;
  }
  state.selected = sel;
  sound('click');
  render();
...
function handView(cards) {
  ...
  for (const grp of state.groups) {
    ...
    // 组牌必须完整保留：只要组内牌不齐（打出过/新开局），整个组移除，不残留半组
    if (kept.length === grp.cards.length) {
      grouped.push({ type: grp.type || '', cards: kept });
      survivingGroups.push({ type: grp.type || '', cards: kept });
    }
    // 否则（该组有牌已打出/不在手牌）→ 整个组移除，组牌区不留残牌
  }
...
function soloAfterUpdate() {
  ...
  if (g.roundEnd && !g.episodeEnd) {
    // 防重复：同一局只庆祝一次（soloAfterUpdate 会被多次调用）
    if (state.lastCelebratedRound !== g.round) {
      state.lastCelebratedRound = g.round;
      // 掼蛋组队：0/2 一队、1/3 一队。头游或对家（队友）头游 = 我方队伍赢 → 号角庆祝
      const leader = (Array.isArray(g.winOrderSeats) && g.winOrderSeats.length > 0) ? g.winOrderSeats[0] : -1;
      const iWon = g.humanSeat !== undefined && leader >= 0 && (leader % 2 === g.humanSeat % 2);
      if (iWon) {
        winFanfare();
        speakChinese(leader === g.humanSeat ? '赢了！' : '我们赢了！'); // 自己头游喊"赢了！"，对家头游喊"我们赢了！"
      } else sound('win'); // 对方队伍赢：简单结束音
    }
  }
...
async function soloDriveBots() {
  ...
  try {
    while (state.game && !state.game.roundEnd && !state.game.episodeEnd && !state.game.waitingIsHuman) {
      ...
      checkHistoryPlays(state.game.history, state.game.humanSeat); // 机器人出牌也播音效（炸弹/同花顺/顺子等）
      render();
    }
...
function applyRoomState(r) {
  ...
  if (r.game) {
    ...
    if (r.game.roundEnd && !r.game.episodeEnd && state.lastCelebratedRound !== r.game.round) {
      // 防重复：同一局只庆祝一次（房间轮询 1s 一次）
      state.lastCelebratedRound = r.game.round;
      // 掼蛋组队：0/2 一队、1/3 一队。头游或对家（队友）头游 = 我方队伍赢 → 号角庆祝
      const leader = (Array.isArray(r.game.winOrderSeats) && r.game.winOrderSeats.length > 0) ? r.game.winOrderSeats[0] : -1;
      const iWon = r.viewerSeat !== undefined && leader >= 0 && (leader % 2 === r.viewerSeat % 2);
      if (iWon) {
        winFanfare();
        speakChinese(leader === r.viewerSeat ? '赢了！' : '我们赢了！'); // 自己头游喊"赢了！"，对家头游喊"我们赢了！"
      } else sound('win'); // 对方队伍赢：简单结束音
    }
...
function renderBoard(g, viewerSeat, humanSeat, turnSeat, countdown, roomSeats) {
  ...
  const mkCard = (c, onClick) => {
    const p = cardParts(c);
    const shown = selShown[c] || 0;
    const isSel = shown < (state.selected[c] || 0); // 同名牌按实例逐个高亮（两张 3♠ 可单独选一张）
    selShown[c] = shown + 1;
    const card = el('span', 'gd-card ' + colorClass(c) + (isSel ? ' sel' : '') + (c === wild ? ' gd-wild' : ''), '', onClick || (() => (inRoom ? roomToggle : soloToggle)(c)));
  ...
  view.groups.forEach((grp, gi) => {
    ...
    grp.cards.forEach((c) => wrap.appendChild(mkCard(c, () => selectGroupPlay(grp))));
