...
安装并重启 web 后，每个会话输入框上方会出现「🃏 掼蛋-中联储卫」条带：

- **展开**牌桌：显示三家对手（张数）、当前最大牌、最近出牌、你的手牌（按点数分组、可点击选中）
...
- **V2（2.0.0）**：图形牌桌 UI（双端：工具 + `/guandan/*` 路由 + 浏览器 dock 面板）
- **V1（1.0.0）**：对话工具版首发

变更见 [CHANGELOG.md](CHANGELOG.md)。后续 **V3** 将在此基础上迭代更新（SSE 实时推送 / 牌桌动画 / better-sidebar 右栏集成等）。
